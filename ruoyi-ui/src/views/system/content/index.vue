<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="所有者ID" prop="owner">
        <el-input
      ...
      <el-form-item label="内容组ID" prop="categoryId">
        <el-input
          v-model="queryParams.categoryId"
          placeholder="请输入内容组ID"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="contentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="所有者ID" align="center" prop="owner" />
      <el-table-column label="内容组ID" align="center" prop="categoryId" />
      <el-table-column label="创建时间" align="center" prop="createDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.createDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="摘要" align="center" prop="resume" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="所有者ID" prop="owner">
          <el-input v-model="form.owner" placeholder="请输入所有者ID" />
        </el-form-item>
        <el-form-item label="内容组ID" prop="categoryId">
          <el-input v-model="form.categoryId" placeholder="请输入内容组ID" />
        </el-form-item>
        <el-form-item label="创建时间" prop="createDate">
          <el-date-picker clearable
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        categoryId: [
          { required: true, message: "内容组ID不能为空", trigger: "blur" }
        ],
        createDate: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除html页面编号为"' + ids + '"的数据项？').then(function() {
